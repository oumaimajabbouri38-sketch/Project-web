# 🌐 Fit With You

A modern multi-page fitness website that promotes a healthy lifestyle.  
Developed as part of a web development learning project using HTML, CSS, and JavaScript.

---

## 🌟 Project Overview

Fit With You is designed to encourage users to adopt a healthy lifestyle by providing useful content about fitness, health, running, and nutrition.

---

## 📄 Pages Included

- 🏠 Home Page (modern and clean design)
- 🏋️ Fitness Training
- 💚 Health & Wellness Tips
- 🏃 Running Guide
- 🍎 Nutrition & Diet
- 📩 Contact Page

---
## 💻 Technologies Used
![HTML5](https://img.shields.io/badge/HTML5-%23FF5722?style=for-the-badge&logo=html5&logoColor=white)
![CSS](https://img.shields.io/badge/CSS-%23264DE4?style=for-the-badge&logo=css&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-%23F7DFE1?style=for-the-badge&logo=javascript&logoColor=black)
![GitHub](https://img.shields.io/badge/GitHub-%23181717?style=for-the-badge&logo=github&logoColor=white)
![Git](https://img.shields.io/badge/Git-%23F05032?style=for-the-badge&logo=git&logoColor=white)
![VS Code](https://img.shields.io/badge/VSCode-0078d7?style=for-the-badge&logo=visual-studio-code&logoColor=white)

## ⚙️ Features

- 🌐 Multi-page structure  
- 🎨 Clean and modern UI  
- 🔗 Smooth navigation  
- 📱 Basic responsive design  
- ⚡ JavaScript interactivity  
- 💪 Fitness-focused content  

---

## 👨‍💻 Team Members

- Oumaima Jabbouri (oumaimajabbouri38-sketch)  
- Ilyass Toufalla (ilyasstoufalla)  
- Soulaimane El Ouafig (sestrongnetwork-lab)  
- Ilyass Sabir (ilyass648)  

---

## 🚀 Project Purpose

This project demonstrates:

- HTML, CSS, JavaScript fundamentals  
- Multi-page website development  
- Team collaboration  
- Basic UI/UX principles  

---

## 🔮 Future Improvements

- 📱 Improve mobile responsiveness  
- ✨ Add animations and transitions  
- 🎨 Enhance UI design  
- 📩 Add backend for contact form  

---

## 📌 Note

This project is for educational purposes only.

---

Stay fit, stay healthy with *Fit With You*!💪
